...
con <- dbConnect(odbc::odbc(),
                 ...
                 Server = "10.11.100.103", 
                 Database = "Maxwarehouse",
                 Trusted_Connection = "Yes")
```

```{r}
historico <- dbGetQuery(con, "SELECT * FROM historico_pesos_nuevo_modelo2")
```
...
```{r}
#historico <- read.csv("df_historico.csv")
historico <- read.csv(text = od$download_file("Shipping\Weights Process\df_historico.csv"))
predicciones <- read.csv("df_predicciones.csv")
metricas <- read.csv("df_metrica.csv")
```
...
download.file(url_hist, temp_file, mode = "wb")
historico <- read.csv(temp_file)
...
temp_file <- tempfile(fileext = ".csv")
download.file(url_pred, temp_file, mode = "wb")
predicciones <- read.csv(temp_file)
...
temp_file <- tempfile(fileext = ".csv")
download.file(url_met, temp_file, mode = "wb")
metricas <- read.csv(temp_file)
...
historico %>% filter(Sku == "ACE-10035")
...
server <- function(input, output, session) {
  ...
  observe({
    updateSelectizeInput(session, "uom", 
                         choices = unique(historico()$UomCode))})
  ...
  # Filtrar datos históricos según el SKU seleccionado
  ...
  # Filtrar predicción según el SKU seleccionado
  ...
  output$hist_table <- renderDT({
    datatable(
      filtered_hist() %>%
        select(TranDate, InvDate, trackingnum, Sku, Weight, 
               Height, Length, Width),
      extensions = 'Buttons',
      options = list(
        dom = 'lBfrtip',
  ...
  content = function(file) {
    write.csv(filtered_hist() %>%
      select(TranDate, InvDate, trackingnum, Sku, 
             Weight, Height, Length, Width),
              file, row.names = FALSE)
    }
  )
  
  # Histogramas de datos historicos
  ...
  # Tabla Predicciones Originales
  output$pred_table_original <- renderTable({
    filtered_pred() %>% select(Sku, Weight, Height, Length, Width)
  })
  
  # Tabla Limite Inferior
  output$LL_table <- renderTable({
    df1 <- filtered_pred() %>% 
      select(Sku, EJD_Weight, EJD_Height, EJD_Length, EJD_Width)
    setNames(df1, c("Sku", "LL Weight", "LL Height", "LL Length", "LL Width"))
  })
  
  # Tabla Limite Superior
  output$UL_table <- renderTable({
    df2 <- filtered_pred() %>% 
      select(Sku, th_weight, th_height, th_length, th_width)
    setNames(df2, c("Sku", "UL Weight", "UL Height", "UL Length", "UL Width"))
  })

  # Tabla Predicciones Ajustadas
  output$pred_table_ajustada <- renderTable({
    df3 <- filtered_pred() %>% 
      select(Sku, Final_Weight, Final_Height, Final_Length, Final_Width)
    setNames(df3, c("Sku", "Final Weight", "Final Height", 
                    "Final Length", "Final Width"))
  })
  
  #Boton descarga predicciones
  ...
  content = function(file) {
    write.csv(filtered_pred() %>% 
                select(Sku, Final_Weight, Final_Height, 
  ...
  filtered_compliance <- reactive({
    ...
    if (!is.null(input$weight_flag_filter) && 
        length(input$weight_flag_filter) > 0) {
      data <- data %>% filter(Weight_Flag %in% input$weight_flag_filter)
    }
    
    if (!is.null(input$dim_flag_filter) && 
        length(input$dim_flag_filter) > 0) {
